...
namespace testConsole
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in totalSleep)
            {
                for (int i = 0; i < 60; i++)
                {
                ...
                sleepiestMinuteCount = sleepMinutes.Aggregate((x, y) => x.Value > y.Value ? x : y).Value;

                if (sleepiestMinuteCount > maxMinute)
                {
                    Console.WriteLine(item.Key);
                    Console.WriteLine(sleepiestMinute);
                    Console.WriteLine(sleepiestMinuteCount);//Last Console entry contains the answer
                    maxMinute = sleepiestMinuteCount;
                }
