...
namespace ConsoleApp1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                u.CalculatePower();
                Console.Write($"{u.count} units each with {u.hp} hit points ");
                if (u.weakTo.Count > 0 || u.immuneTo.Count > 0)
                {
                    Console.Write("(");
                    if (u.weakTo.Count > 0)
                    {
                        Console.Write("WEAK TO: ");
                        foreach (string s in u.weakTo)
                        {
                            Console.Write(s + " ");
                        }
                    }
                    if (u.immuneTo.Count > 0)
                    {
                        Console.Write("IMMUNE TO: ");
                        foreach (string s in u.immuneTo)
                        {
                            Console.Write(s + " ");
                        }
                    }
                ...
                Console.Write($" with an attack that does {u.damage} {u.damagetype} at initiative {u.initiative}");
                Console.WriteLine($"    {u.power}");
            }
            Console.WriteLine();
            ...
            {
                #region Unit Counts
                Immune.team = Immune.team.OrderByDescending(x => x.power).ThenByDescending(x => x.initiative).ToList();//.ThenByDescending(x => x.initiative);
